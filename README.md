...
| 跨会话没有记忆 | **持久记忆**和人格设定 |
| 只能在单一平台使用 | 支持 **8 个平台**（DeepSeek 最佳） |
...
| ChatGPT | ⚠️ 基本可用 | GPT-4 / GPT-4 Turbo，部分场景需调试 |
| Kimi | ⚠️ 基本可用 | Moonshot v1 8K/32K/128K，流式解析待优化 |
| 通义千问 | ⚠️ 基本可用 | 千问国内版，输入框交互待完善 |
| Qwen International | ⚠️ 基本可用 | chat.qwen.ai，输入框交互待完善 |
| 智谱清言 | ⚠️ 基本可用 | glm-4-Plus，部分 DOM 选择器待适配 |
| 智谱国际版 (z.ai) | ⚠️ 基本可用 | GLM-4 Plus，部分 DOM 选择器待适配 |
| 豆包 | ⚠️ 实验性 | doubao-seed-2.0，流式解析存在限制 |

> **提示**：本项目以 DeepSeek 为主要开发和测试平台，其他平台的适配仍在持续完善中。如果你主要使用 DeepSeek，可以获得最佳体验。
